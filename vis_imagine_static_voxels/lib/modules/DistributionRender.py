...
class DistributionRender_Z(tf.keras.Model):
  def __init__(self, hiddim):
    super(DistributionRender_Z, self).__init__()
    self.render_mean = tf.keras.Sequential([
                      tf.keras.layers.Conv3D(hiddim, 3, 1, "same"),
                      tf.keras.layers.Activation("elu"),
                      tf.keras.layers.Conv3D(hiddim, 3, 1, "same"),
  ...
  @tf.function
  def call(self, x):
    mean = self.render_mean(x)
    return mean

def run():
  x1 = [tf.random.normal([1,16,16,16,3]),tf.random.normal([1,16,16,16,3])]
  import time
  s = time.time()
  dr(x1)
  print(time.time() - s)
