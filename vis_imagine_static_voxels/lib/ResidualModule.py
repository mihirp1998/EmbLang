...
class ResidualBlock(tf.keras.layers.Layer):
    def __init__(self, resample, nonlinear, ifgate, indim, outdim, normalize):
        ...
        if ifgate:
            self.conv1 = tf.keras.layers.Conv3D(outdim,3,1,padding="same")
            self.conv2 = tf.keras.layers.Conv3D(outdim,3,1,padding="same")
    ...
    def conv(self, name, indim, outdim, normalize=None):
        if name == 'sconv_d':
            if normalize == 'weight_norm':
              return tf.keras.layers.Conv3D(outdim,4,2,padding="same")
            else:
              return tf.keras.layers.Conv3D(outdim,4,2,padding="same")
        elif name == 'sconv':
            if normalize == 'weight_norm':
              return tf.keras.layers.Conv3D(outdim,3,1,padding="same")
            else:
              return tf.keras.layers.Conv3D(outdim,3,1,padding="same")
        elif name == 'upconv':
            if normalize == 'weight_norm':
              return tf.keras.layers.Conv3DTranspose(outdim,4,2,padding="same")
            else:
              return tf.keras.layers.Conv3DTranspose(outdim,4,2,padding="same")
        else:
...
if __name__ == "__main__":
    ...
    print(resMod(tf.zeros([1, 30, 30, 30, 10]),training=True))
