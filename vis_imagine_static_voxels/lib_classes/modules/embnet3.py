...
class embnet3(tf.keras.Model):
    def __init__(self,istrain):
        super(embnet3, self).__init__()
    ...
    def margin_loss_3D(self,emb0, emb1):
    ...
    def call(self,emb_pred, emb):
        ...
        with tf.compat.v1.name_scope('emb'):
            ...
            loss_3D = self.margin_loss_3D(emb, emb_pred)
            total_loss = add_loss(total_loss, loss_3D,
                                             const.emb_coeff, '3D')

            loss_3D_l1 = l1_on_axis(emb-emb_pred, axis=4)
            # utils_improc.summ_oned('3D_l1_loss', tf.reduce_mean(loss_3D_l1, axis=1))
            loss_3D_l1 = tf.reduce_mean(loss_3D_l1)
            total_loss = add_loss(total_loss, loss_3D_l1,
                                             const.emb_l1_coeff, '3D_l1')
