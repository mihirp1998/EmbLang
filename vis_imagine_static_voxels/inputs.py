    ...
    def data_for_selector(self, selector):
        ...
        if const.eager:
            return [self.train_data.get_next,
                    ...
                    self.test_data.get_next][selector]()
        else:
            rval = tf.case({
...
class MNISTInput(Input):
    def __init__(self):
        super().__init__()
        self.q_ph = None
        raise Exception('Load MNIST data from new location in tf 2.0')
...
class ShapenetInput(TFDataInput):
    def __init__(self):
        ...
        self.test_data = self.make_data(const.test_file, False, False)
    ...
    def make_data(self, fn, shuffle, istest = False):
        ...
        # st()
        if fn == "double_train":
            # fns = glob.glob("CLEVR_64_TRAIN/tfrs/*")
            if const.OVERFIT_ON_SUBSET:
                fns = glob.glob("/home/mprabhud/merger/tf2_3dmapping/CLEVR_64_36_WITH_INSIDE/tfrs_withnames/*")
            else:    
                # fns = glob.glob("/home/mprabhud/merger/tf2_3dmapping/CLEVR_64_36_WITH_INSIDE/tfrs_withnames/*")
                fns = pickle.load(open("CLEVR_64_36_AFTER_CORRECTION_NO_DEPTH/single_obj.p","rb"))
        ...
        if fn == "double_test":
            fns = glob.glob("CLEVR_64_36_AFTER_CORRECTION_NO_DEPTH_TEST/tfrs_withnames/*")
            fns = pickle.load(open("CLEVR_64_36_AFTER_CORRECTION_NO_DEPTH/single_obj.p","rb"))
            # fns = glob.glob("/home/sajaved/projects/text2scene/3dProbNeuralProgNet/data/CLEVR/clevr-dataset-gen/output/CLEVR_64_GEN_TEST/tfrs/*")
        # st()
...
class MultiViewInput(ShapenetInput):
    ...
    def decode(self, example):
        # feature_names = ['images', 'obj_mask',"treefile_path"]
        feature_names = ['images',"treefile_path","mask"]
        ...
        file_name = tf.constant("tempfilename")
        tree_filename = stuff["treefile_path"]
        ...
        obj_mask = tf.io.decode_raw(stuff['mask'], tf.float32)
        obj_mask = tf.reshape(obj_mask, (N, const.Hdata, const.Wdata, 1))

        # obj_mask = tf.ones((N, const.Hdata, const.Wdata, 1))

        rvals = [images, masks, obj_mask, file_name,tree_filename]
        ...
        if const.MASKED_DATASET:
            valids = tf.io.decode_raw(stuff['valids'], tf.uint8)
...
class MultiViewReconstructionInput2(MultiViewInput):
    ...
    def normalize(self, images, masks, obj_mask, file_name,tree_filename, *args):
        ...
        def extract_view_(images, masks, obj_mask, idx):
            offset_tensor = tf.stack([idx, 0, 0, 0])
            size_tensor = [1, -1, -1, -1]
            extract = lambda x: tf.slice(x, offset_tensor, size_tensor)
            return extract(images), extract(masks), extract(obj_mask)

        def extract_rand_view(images, masks, obj_mask,i, seed = 0):
            # st()
            if not const.FIX_VIEW:
                ...
                if not const.MASKED_DATASET:
                    ...
                    theta_idx = tf.compat.v1.div(phi_theta_idx, const.VV)
            else:
                if const.MASKED_DATASET:
            ...
            idx = phi_idx * const.HV + theta_idx
            return extract_view_(images, masks, obj_mask, idx), (phi_idx, theta_idx, idx)
        # st()
        random.seed(2)
        # views, idxs = list(zip(*[
        #     extract_rand_view(images, masks, obj_mask, seed = random.randint(0,100))
        #     for _ in range(const.NUM_VIEWS + const.NUM_PREDS) 
        # ]))
        views, idxs = list(zip(*[
            extract_rand_view(images, masks, obj_mask,i, seed = random.randint(0,100))
            for i in range(const.NUM_VIEWS + const.NUM_PREDS) 
        ]))


        images, masks, obj_mask = list(zip(*views))
        phi_idx, theta_idx, idx = list(zip(*idxs))

        images = [img/255.0 for img in images]
        masks = [mask/255.0 for mask in masks]
        obj_mask = [(zmap + const.MASKWEIGHT)/(1 + const.MASKWEIGHT) for zmap in obj_mask]
        # obj_mask = [zmap*2.0 for zmap in obj_mask]
        ...
        images = [tf.reshape(img, (const.Hdata, const.Wdata, 3)) for img in images]
        masks = [tf.reshape(mask, (const.Hdata, const.Wdata, 1)) for mask in masks]
        obj_mask = [tf.reshape(zmap, (const.Hdata, const.Wdata, 1)) for zmap in obj_mask]

        images = tuple(images)
        masks = tuple(masks)
        obj_mask = tuple(obj_mask)
        ...
        names = ['images', 'masks', 'obj_mask', 'phis', 'thetas','file_name','tree_filename']
        stuff = [images, masks, obj_mask, phi_idx, theta_idx, file_name, tree_filename]
...
class GQNShapenet:
    ...
    def data(self,index= None):
        data = self.child.data_for_selector(selector=index)
        # st()
        data = Munch(query = self.make_query(data),
                    target_mask = self.make_target_mask(data),
                     target = self.make_target(data),file_name=data.file_name,treefile_path=data.tree_filename)
        return data
