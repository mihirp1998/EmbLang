...
def constant_boolean_mask_single(xxx_todo_changeme, k):
    ...
    tensor = tf.boolean_mask(tensor=tensor, mask=mask)
...
def sample_with_mask(tensor, mask, sample_count):
    ...
    k = tf.minimum(tf.cast(tf.reduce_min(input_tensor=tf.reduce_sum(input_tensor=hard_float_mask, axis = 1)), tf.int32), sample_count)
    feats = constant_boolean_mask(tensor, hard_mask, k)
    return feats
        
def sample_indices(k, N):
    randoms = tf.random.uniform(shape = tf.stack([N]))
