...
def transformer(im, flow, out_size, name='SpatialTransformer', **kwargs):
    ...
    def _interpolateHW(im, x, y, out_size):
        with tf.compat.v1.variable_scope('_interpolateHW'):
            # constants
            nBatch = tf.shape(input=im)[0]
            ...
            d_a = tf.compat.v1.sparse_to_dense(tf.cast(idx_a, 'int32'),
            ...
            d_b = tf.compat.v1.sparse_to_dense(tf.cast(idx_b, 'int32'),
                                     [nBatch * height * width], 1,
            ...
            d_c = tf.compat.v1.sparse_to_dense(tf.cast(idx_c, 'int32'),
                                     [nBatch * height * width], 1,
            ...
            d_d = tf.compat.v1.sparse_to_dense(tf.cast(idx_d, 'int32'),
                                     [nBatch * height * width], 1,
    ...
    def _meshgridHW(height, width):
        with tf.compat.v1.variable_scope('_meshgridHW'):
...
def depth_and_rot_to_flow(Z, r, distance=4.0):
    ...
    XYZ_transpose = tf.transpose(a=XYZ, perm=[0, 2, 1])
    XYZ_mm = tf.matmul(r, XYZ_transpose)
    XYZ_rot = tf.transpose(a=XYZ_mm, perm=[0, 2, 1])
