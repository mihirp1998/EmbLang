...
def transformer(voxels,
    ...
    def _noproj_meshgrid(depth, height, width, z_near, z_far):
        with tf.compat.v1.variable_scope('_meshgrid'):
            x_t = tf.reshape(
                tf.tile(tf.linspace(-const.SCENE_SIZE, const.SCENE_SIZE, width), [height * depth]),
                [depth, height, width])
            y_t = tf.reshape(
                tf.tile(tf.linspace(-const.SCENE_SIZE, const.SCENE_SIZE, height), [width * depth]),
                [depth, width, height])
            y_t = tf.transpose(a=y_t, perm=[0, 2, 1])
    ...
    def _transform(theta, input_dim, out_size, z_near, z_far):
        with tf.compat.v1.variable_scope('_transform'):
            ...
            if do_project is not "invert":
                ...
                y_s = y_s * (1/const.SCENE_SIZE)
                x_s = x_s * (1/const.SCENE_SIZE)
            else:
                z_s = z_s/const.SCENE_SIZE
