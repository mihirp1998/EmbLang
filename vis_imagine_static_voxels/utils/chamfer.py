...
def normalized_visibility(coords, nb=10):
    ...
    idx_map = tf.compat.v1.assign(idx_map, idx_zero, validate_shape=False)
    ...
    for coords, idxs in zip(binned_coords, binned_idxs):
        ...
        idx = tf.boolean_mask(tensor=idx, mask=valid)
        vals_to_scatter = tf.boolean_mask(tensor=idxs, mask=valid)

        idx_map = tf.compat.v1.scatter_update(idx_map, idx, vals_to_scatter)

    vis_idx = tf.boolean_mask(tensor=idx_map, mask=idx_map > 0) - 1  # subtract off the offset
...
def chamfer_preprocess(pts):
    ...
    alpha = pts[:, 6]
    keep_rnd = tf.squeeze(tf.where(tf.random.uniform(tf.shape(input=alpha)) < alpha), axis=1)
...
def dist_mat(pts1, pts2):
    #(x-y)^2 = x^2 + y^2 - 2xy
    xy = tf.matmul(pts1, tf.transpose(a=pts2))  # n by m
    xsq = tf.expand_dims(tf.reduce_sum(input_tensor=tf.square(pts1), axis=1), axis=1)  # n by 1
    ysq = tf.expand_dims(tf.reduce_sum(input_tensor=tf.square(pts2), axis=1), axis=0)  # 1 by m
    return xsq + ysq - 2 * xy
...
def min_dist(pts1, pts2):
    ...
    return tf.argmin(input=distmat, axis=0)
