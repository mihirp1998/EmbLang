...
def scatter_add_tensor(ref, indices, updates, name=None):
    ...
    """
    with tf.compat.v1.name_scope(name, 'scatter_add_tensor', [ref, indices, updates]) as scope:
        ref = tf.convert_to_tensor(value=ref, name='ref')
        indices = tf.expand_dims(tf.convert_to_tensor(value=indices, name='indices'), axis=1)
        updates = tf.convert_to_tensor(value=updates, name='updates')
        ref_shape = tf.shape(input=ref, out_type=indices.dtype, name='ref_shape')
        scattered_updates = tf.scatter_nd(indices, updates, ref_shape, name='scattered_updates')
        with tf.control_dependencies(
                [tf.compat.v1.assert_equal(ref_shape, tf.shape(input=scattered_updates, out_type=indices.dtype))]
        ):
...
def bilinear_scatter(ref, indices, weights, quantity=1.0):
    ...
    ref = tf.compat.v1.scatter_add(ref, indices, weights)
    return ref
...
def render0(pts):
    ...
    _min_z = tf.reduce_min(input_tensor=Z)
    _max_z = tf.reduce_max(input_tensor=Z)
    ...
    img = tf.compat.v1.assign(img, img_zero, validate_shape=False)
    norm = tf.compat.v1.assign(norm, ch_zero, validate_shape=False)
    depth = tf.compat.v1.assign(depth, ch_zero, validate_shape=False)
    alpha = tf.compat.v1.assign(alpha, ch_zero, validate_shape=False)
    alpha_norm = tf.compat.v1.assign(alpha_norm, ch_zero, validate_shape=False)
...
def render_pts_from_pose(pts, theta, phi, do_preprocess=False):
    ...
    with tf.compat.v1.name_scope('render'):
        out = render1(pts_)
...
def reproject_all(views, depths, masks):
    with tf.compat.v1.name_scope('reproject_lift'):
    ...
    with tf.compat.v1.name_scope('reproject_sink'):
